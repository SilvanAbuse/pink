...
<div class="<?php echo esc_attr( $video_shortcode_classes ); ?>">
	<div class="pgs-video"> 	
		<?php if ( ! empty( $opacity_color ) && ( 'true' == $enable_opacity ) ) { ?>
		...
		<div class="pgs-video-content video_popup-gallery">
			...
			if ( ! empty( $video_link ) ) {
				?>
				<?php if ( ! empty( $icon_style ) ) { ?>
					<a class="popup-youtube" href="<?php echo esc_url( $video_link ); ?>" > 
						<img class="img-responsive" src="<?php echo $icon_style; ?>" alt="Video Img"> 
					</a>
					<?php } ?>	
			...
			if ( ! empty( $title ) ) {
				...
				<div class="pgs-title-des">
					...
					if ( true == $show_content ) {
						...
						<<?php echo $title_element; ?> class="pgs-video-title">
							<?php echo $title; ?>
