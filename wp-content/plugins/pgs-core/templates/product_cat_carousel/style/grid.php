...
$category_title_styles = $category_hover_title_styles = array();
...
if ( $style != 'style-2' ) {
	if ( isset( $background_overlay ) && $background_overlay != 'none' ) {
		if ( $background_overlay == 'custom' ) {
			if ( $style == 'style-1' ) {
				if ( isset( $category_background_color ) && ! empty( $category_background_color ) ) {
				...
				<div class="pgs-core-category-container">
					...
					<?php } ?>
						<div class="category-content" style="<?php echo esc_attr( implode( '', $cat_overlay_style ) ); ?>">	
							<?php
							if ( $title_counter_display == 'counter-up-title-bottom' ) {
								?>
									<?php if ( ! $hide_categories_count ) { ?>
									<span style="<?php echo esc_attr( implode( '', $category_count_styles ) ); ?>" class="product-count" >
										<?php echo $category->count; ?> <?php esc_html_e( 'Products', 'pgs-core' ); ?>
									</span>
										<?php
									}
									if ( ! empty( $category_title ) ) {
										?>
										<<?php echo $tag; ?> class="category-title">
