...
if ( ! empty( $add_icon ) && 'true' == $add_icon && ! empty( $icon_html ) ) {
...
if ( $list_font_weight && ! empty( $list_font_weight ) ) {
	$element_tag_style[] = "font-weight:{$list_font_weight};";
}
if ( $list_text_transform && ! empty( $list_text_transform ) ) {
	$element_tag_style[] = "text-transform:{$list_text_transform};";
}
if ( $list_title_color && ! empty( $list_title_color ) ) {
	...
	foreach ( $list_items as $list_item ) {
		if ( isset( $list_item['content'] ) && ! empty( $list_item['content'] ) ) {
			...
			<li>
				...
				<?php echo ( ! empty( $add_icon ) && 'true' == $add_icon && ! empty( $icon_html ) ) ? wp_kses( $icon_html, pgscore_allowed_html( array( 'i', 'span' ) ) ) . ' ' : ''; ?>
				<?php if ( '' != $link_attr ) { ?> 

					<<?php echo esc_attr( $list_element_tag ); ?> class="pgscore-list-info inline_hover" style="<?php echo esc_attr( $element_tag_style ); ?>" data-hover_styles="<?php echo esc_attr( json_encode( $hover_text_styles ) ); ?>" data-prehover_style="<?php echo esc_attr( json_encode( $prehover_style ) ); ?>">
					<?php
					if ( '' != $link_attr ) {
						?>
					...
					?>
					</<?php echo esc_attr( $list_element_tag ); ?>>

				<?php } else { ?>
					<<?php echo esc_attr( $list_element_tag ); ?> class="pgscore-list-info " style="<?php echo esc_attr( $element_tag_style ); ?>">
