...
if ( ! empty( $portfolio_type ) && $portfolio_type == 'isotope' ) {
	$portfolio_classes[]         = 'isotope';
	$portfolio_section_classes[] = 'isotope-wrapper';
}

if ( ! empty( $portfolio_type ) && ( $portfolio_type == 'isotope' || $portfolio_type == 'grid' ) ) {
...
<div class="<?php echo esc_attr( $portfolio_section_classes ); ?>">
	<?php
	if ( $portfolio_type == 'isotope' && ! is_wp_error( $filter_terms ) && is_array( $filter_terms ) && ! empty( $filter_terms ) ) {
		?>
