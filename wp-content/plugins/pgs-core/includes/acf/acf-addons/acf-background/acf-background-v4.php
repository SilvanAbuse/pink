...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	*/
	...
	function create_field( $field ) {
		...
		if ( 1 == $field['background_color'] ) {
			...
			if ( 1 == $field['background_repeat'] or 1 == $field['background_position'] or 1 == $field['background_attachment'] ) {
				echo '<br />';
			}
		}

		if ( 1 == $field['background_repeat'] ) {
			$array = array(
				'no-repeat' => 'No Repeat',
				'repeat'    => 'Repeat All',
		...
		if ( 1 == $field['background_attachment'] ) {
			$array = array(
				'scroll'  => 'Scroll',
				'fixed'   => 'Fixed',
				'local'   => 'Local',
		...
		if ( 1 == $field['background_position'] ) {
			$array = array(
				'left top'      => 'Left Top',
		...
		if ( 1 == $field['background_image'] ) {
			...
			if ( ( isset( $field['preview_media'] ) && 0 == $field['preview_media'] ) ) {
				$field['class'] .= ' noPreview';
			}

			if ( ( ! empty( $field['background-image'] ) && 1 == $field['background-image'] ) || isset( $field['preview'] ) && 0 == $field['preview'] ) {
				$hide = '';
			}
			...
			if ( ( 1 == $field['preview_media'] && 0 == $field['preview_media'] ) || ( '' == $value['background-image'] ) ) {
				$hide = 'hide ';
			}
			...
			echo '<img class="rey-option-image" id="image_' . $value['media']['id'] . '" src="' . esc_url( $value['media']['thumbnail'] ) . '" alt="" target="_blank" rel="external" />';
			echo '</a>';
			echo '</div>';
		...
		if ( ! isset( $field['preview'] ) || 0 != $field['preview'] ) {
	...
	function getCSS( $field ) {
		...
		if ( ! empty( $field['value'] ) ) {
			foreach ( $field['value'] as $key => $value ) {
				if ( ! empty( $value ) && 'media' != $key && 'color-transparency' != $key ) {
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		wp_register_script( 'acf-input-background', $this->settings['dir'] . 'js/field_background' . $suffix . '.js', array( 'acf-input' ), $this->settings['version'] );
		wp_register_style( 'acf-input-background', $this->settings['dir'] . 'css/field_background' . $suffix . '.css', array( 'acf-input' ), $this->settings['version'] );
