...
if ( ! function_exists( 'header_builder_html_wishlist' ) ) {
	function header_builder_html_wishlist( $args ) {
		...
		if ( empty( $args ) || ! class_exists( 'YITH_WCWL' ) ) {
			return;
		}
...
if ( ! function_exists( 'header_builder_html_button' ) ) {
	function header_builder_html_button( $args ) {
		...
		if ( isset( $el_option['button_title'] ) && ! empty( $el_option['button_title'] ) ) {
			?>
			<div <?php echo esc_attr( $wrapper_id ); ?> class="<?php echo esc_attr( $wrapper_class ); ?>">
				<a href="<?php echo esc_attr( $el_option['button_link'] ); ?>" <?php echo ( $el_option['button_new_tab'] ) ? 'target="_blank"' : ''; ?>>
					<?php echo esc_html( $el_option['button_title'] ); ?>
				</a>
			</div>
			<?php
		}
			...
			if ( ! is_user_logged_in() ) {
				if ( class_exists( 'WooCommerce' ) ) {
					if ( isset( $el_option['show_register_form'] ) && $el_option['show_register_form'] ) {
						$account_html = '<a href="javascript:void(0);" data-toggle="modal" data-target="#pgs_login_form">' . $ciyashop_account_icon . ' ' . esc_html__( $el_option['account_text'], 'pgs-core' ) . '</a>';
					} else {
						$account_html = '<a href="' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . '">' . $ciyashop_account_icon . ' ' . esc_html__( $el_option['account_text'], 'pgs-core' ) . '</a>';
					}
				} else {
					$account_html = '<a href="' . wp_login_url( home_url() ) . '" title="Login">' . $ciyashop_account_icon . ' ' . esc_html__( $el_option['account_text'], 'pgs-core' ) . '</a>';
				}
				...
				if ( class_exists( 'WooCommerce' ) ) {
					...
					$account_html .= '<a href="' . get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) . '" title="' . esc_attr__( 'My Account', 'pgs-core' ) . '" class="myaccount">' . esc_html__( 'My Account', 'pgs-core' ) . '</a>';
					$account_html .= '</div>';
				}
