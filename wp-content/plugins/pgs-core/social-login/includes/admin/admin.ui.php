...
function pgssl_admin_main() {
	...
	$pgssl_page      = "networks";
	$assets_base_url = PGS_SOCIAL_LOGIN_PLUGIN_URL . 'assets/img/16x16/';
	...
	<div class="wrap">
		...
		<h2><?php esc_html_e( 'PGS Social Login', 'pgssl-text-domain' ); if( $tutorial_link ) { ?> <a class="add-new-h2" target="_blank" href="<?php echo esc_url($tutorial_link); ?>">Read Tutorial</a><?php } ?></h2>
		...
		<div class="pgssl-settings-meta-box-wrap">
			...
			if( in_array( $pgssl_page, array( 'networks' ) ) and ( isset( $_REQUEST['settings-updated'] ) or isset( $_REQUEST['enable'] ) ) ) {
				...
				if( $cache_enabled ) {
					?>
						<div class="fade updated" style="margin: 4px 0 20px;">
							<p>
								<?php _pgssl_e("<b>Note:</b> PGS Social Login has detected that you are using a caching plugin. If the saved changes didn't take effect immediately then you might need to empty the cache", 'pgssl-text-domain') ?>.
							</p>
						</div>
					<?php
				}
			...
			if( get_option( 'pgssl_settings_development_mode_enabled' ) ) {
				...
				<div class="fade error pgssl-error-dev-mode-on">
					<p>
						<?php _pgssl_e('<b>Warning:</b> You are now running PGS Social Login with DEVELOPMENT MODE enabled. This mode is not intend for live websites as it might raise serious security risks', 'pgssl-text-domain') ?>.
					</p>
					<p>
						<a class="button-secondary" href="<?php echo esc_url($pgssl_menu_path) ;?>?page=pgssl_settings&pgssl_page=tools#dev-mode"><?php _pgssl_e('Change this mode', 'pgssl-text-domain') ?></a>
						<a class="button-secondary" href="http://miled.github.io/wordpress-social-login/troubleshooting-advanced.html" target="_blank"><?php _pgssl_e('Read about the development mode', 'pgssl-text-domain') ?></a>
					</p>
				</div>
				<?php
			}
			...
			if( get_option( 'pgssl_settings_debug_mode_enabled' ) ) {
				...
				<div class="fade updated pgssl-error-debug-mode-on" style="margin: 4px 0 20px;">
					<p>
						<?php _pgssl_e('<b>Note:</b> You are now running PGS Social Login with DEBUG MODE enabled. This mode is not intend for live websites as it might add to loading time and store unnecessary data on your server', 'pgssl-text-domain') ?>.
					</p>
					<p>
						<a class="button-secondary" href="<?php echo esc_url($pgssl_menu_path) ;?>?page=pgssl_settings&pgssl_page=tools#debug-mode"><?php _pgssl_e('Change Debug Mode', 'pgssl-text-domain') ?></a>
						<a class="button-secondary" href="<?php echo esc_url($pgssl_menu_path) ;?>?page=pgssl_settings&pgssl_page=watchdog"><?php _pgssl_e('View Logs', 'pgssl-text-domain') ?></a>
					</p>
				</div>
				<?php
			}
					...
					foreach( $admin_tabs as $name => $settings ) {
						if( isset($settings["enabled"]) && $settings["enabled"] && ( $settings["visible"] || $pgssl_page == $name ) ) {
							...
							<a class="nav-tab <?php if( $pgssl_page == $name ) echo "nav-tab-active"; ?>" href="<?php echo esc_url($link); ?>">
								<?php _pgssl_e( $settings["label"], 'pgssl-text-domain' );?>
							</a>
							<?php
						}
					}
			...
			<div id="pgssl_admin_tab_content">
				...
				if( isset( $admin_tabs[$pgssl_page] ) && isset($admin_tabs[$pgssl_page]["enabled"]) && $admin_tabs[$pgssl_page]["enabled"] ) {
					if( isset( $admin_tabs[$pgssl_page]["action"] ) && $admin_tabs[$pgssl_page]["action"] ) {
						...
						include "components/$pgssl_page/index.php";
					}
				} else {
...
function pgssl_add_admin_stylesheets() {
	wp_register_style( "pgssl-admin", PGS_SOCIAL_LOGIN_PLUGIN_URL . "assets/css/pgssl-admin.css" );
