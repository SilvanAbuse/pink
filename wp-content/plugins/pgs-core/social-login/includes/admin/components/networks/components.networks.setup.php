...
function pgssl_component_networks_setup() {
	...
	function toggleproviderkeys(idp) {
		...
		if(jQuery('#pgssl_settings_' + idp + '_enabled').val()==1) {
			jQuery('.pgssl_tr_settings_' + idp).show();
		} else {
			jQuery('.pgssl_tr_settings_' + idp).hide();
	...
	foreach( $providers_config AS $item ) {
		...
		if( ! ( ( isset( $item["default_network"] ) && $item["default_network"] && ! get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) || get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) ) {
			continue;
		}
		...
		<div class="postbox pgssl-postbox pgssl-postbox-<?php echo strtolower( $provider_id );?>" id="setup<?php echo strtolower( $provider_id );?>">
			...
			<div class="inside pgssl-fields">
				<table class="form-table editcomment">
					<tbody>
						<tr>
							<td style="width:125px"><?php _pgssl_e("Enabled", 'pgssl-text-domain') ?>:</td>
							<td>
								<select
						...
						<?php if ( $provider_new_app_link ){ ?>
							<?php if ( $require_client_id ){ // key or id ? ?>
								<tr valign="top" <?php if( ! get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="pgssl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _pgssl_e("Application ID", 'pgssl-text-domain') ?>:</td>
									<td><input dir="ltr" type="text" name="<?php echo 'pgssl_settings_' . $provider_id . '_app_id' ?>" value="<?php echo get_option( 'pgssl_settings_' . $provider_id . '_app_id' ); ?>" ></td>
							...
							<?php } else { ?>
								<tr valign="top" <?php if( ! get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="pgssl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _pgssl_e("Application Key", 'pgssl-text-domain') ?>:</td>
									<td><input dir="ltr" type="text" name="<?php echo 'pgssl_settings_' . $provider_id . '_app_key' ?>" value="<?php echo get_option( 'pgssl_settings_' . $provider_id . '_app_key' ); ?>" ></td>
									<?php /*?><td><a href="javascript:void(0)" onClick="toggleproviderhelp('<?php echo $provider_id; ?>')"><?php _pgssl_e("Where do I get this info?", 'pgssl-text-domain') ?></a></td><?php */?>
								</tr>
							<?php }; ?>
							...
							<?php if( ! $require_api_key ) { ?>
								<tr valign="top" <?php if( ! get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="pgssl_tr_settings_<?php echo $provider_id; ?>" >
									<td><?php _pgssl_e("Application Secret", 'pgssl-text-domain') ?>:</td>
									<td><input dir="ltr" type="text" name="<?php echo 'pgssl_settings_' . $provider_id . '_app_secret' ?>" value="<?php echo get_option( 'pgssl_settings_' . $provider_id . '_app_secret' ); ?>" ></td>
									<?php /*?><td><a href="javascript:void(0)" onClick="toggleproviderhelp('<?php echo $provider_id; ?>')"><?php _pgssl_e("Where do I get this info?", 'pgssl-text-domain') ?></a></td><?php */?>
								</tr>
							<?php } ?>
							<tr valign="top" <?php if( ! get_option( 'pgssl_settings_' . $provider_id . '_enabled' ) ) echo 'style="display:none"'; ?> class="pgssl_tr_settings_<?php echo $provider_id; ?>" >
								<td><?php _pgssl_e("Callback URL", 'pgssl-text-domain') ?>:</td>
								<td><em><?php echo $endpoint_url . 'callbacks/' . strtolower($provider_id) . '.php'; ?></em></td>
								<?php /*?><td></td><?php */?>
							</tr>
				...
				<br />
				<div
					class="pgssl_div_settings_help_<?php echo $provider_id; ?>"
