	...
	<ul>
		...
		foreach ( $blog_metas as $blog_meta_k => $blog_meta_v ) {
			...
			if ( 'categories' == $blog_meta_k && ! empty( $blog_meta_v ) && ! empty( $category_list ) ) {
				?>
				<li><span class="entry-meta-categories"><i class="fa fa-folder-open"></i>&nbsp;<?php echo get_the_category_list( trim( $category_list_args['sep'] ) . ' ' ); ?></span></li>
				<?php
			}

			if ( 'tags' == $blog_meta_k && ! empty( $blog_meta_v ) && ! empty( get_the_tag_list() ) ) {
				?>
				<li><span class="entry-meta-tags"><?php echo get_the_tag_list( '<i class="fa fa-tags"></i>', ', ' ); ?></span></li>
				<?php
			}
		}
