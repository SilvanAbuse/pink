...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $product_images_classes ) ) ); ?>">
	...
	<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $product_images_wrapper_classes ) ) ); ?>">
		...
		<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>">
			...
			<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $gallery_wrapper_classes ) ) ); ?>" data-default_image_props="<?php echo htmlspecialchars( wp_json_encode( $default_image_props ) ); ?>">
			<?php
			if ( ! empty( $product_gallery_image_ids ) ) {
				...
				foreach ( $product_gallery_image_ids as $attachment_id ) {
					...
					if ( is_array( $size_class ) ) {
						...
						$attributes  = array(
							...
							'class'                   => "attachment-$size_class size-$size_class" . ( ( 1 == $gallery_images_sr ) ? 'wp-post-image' : '' ),
						);
						...
						echo apply_filters( 'ciyashop_single_product_image_html', $html, $attachment_id );
				}
			} else {
				...
				$html .= '</div>';
				echo apply_filters( 'ciyashop_single_product_placeholder_image_html', $html, $product );
			}
			?>
			</div>
