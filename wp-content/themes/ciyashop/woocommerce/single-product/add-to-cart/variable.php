		...
		?>
			<table class="variations" cellspacing="0">
				...
				foreach ( $attributes as $attribute_name => $options ) {
					...
					// check swatch option is not disabled for attribute
					$enable_swatch = get_option( 'ciyashop_' . $attribute_name . '_swatch' );
					?>
						<tr>
							<td class="label">
								<label for="<?php echo esc_attr( sanitize_title( $attribute_name ) ); ?>"><?php echo ucwords( wc_attribute_label( $attribute_name ) ); // WPCS: XSS ok. ?></label>
							</td>
										...
										if ( is_array( $options ) ) {
											...
											if ( taxonomy_exists( $attribute_name ) ) {
												...
												foreach ( $terms as $term ) {
													...
													if ( isset( $default_attributes[ $attribute_name ] ) && $term->slug == $default_attributes[ $attribute_name ] ) {
