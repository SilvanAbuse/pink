	...
	class Envato_Theme_Setup_Wizard {
		...
		public function setup_wizard_header() {
			...
			<head>
				...
				echo wp_kses( '<ti' . 'tle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'ciyashop' ) . '</ti' . 'tle>', ciyashop_allowed_html( 'title' ) );
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				<pre>
					...
					if ( ! $post_type || ! isset( $all_data[ $post_type ] ) ) {
						echo sprintf( esc_html__( 'Post type %s not found.', 'ciyashop' ), $post_type );
					} else {
						echo sprintf( esc_html__( 'Looking for post id %s', 'ciyashop' ), $post_id ) . "\n";
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the steps for setting the %s theme.', 'ciyashop' ), $this->theme_data->get( 'Name' ) ); ?></h1>
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to %s Setup Wizard', 'ciyashop' ), $this->theme_data->get( 'Name' ) ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing %s theme.', 'ciyashop' ), $this->theme_data->get( 'Name' ) ); ?></p>
				<p><?php printf( esc_html__( 'This setup wizard will help you to refresh and configure your website with a new layout. You will have Child Theme, Content, and Plugins installed in 5-10 minutes (depending on your server configuration). ', 'ciyashop' ), $this->theme_data->get( 'Name' ) ); ?></p>
		...
		public function envato_setup_default_plugins() {
			...
			<form method="post" class="plugins-form" data-version="<?php echo esc_attr( $version_import ); ?>">
				...
				if ( count( $plugins['all'] ) && count( $plugins['required'] ) > 0 ) {
					...
					if ( count( $plugins['all'] ) == 0 && count( ciyashop_tgmpa_plugin_list() ) > 0 && ! ciyashop_is_activated() ) {
						?>
		...
		private function _plugin_list_item( $slug, $plugin, $plugins, $checked = false, $plugin_type ) {
			...
			<li data-slug="<?php echo esc_attr( $slug ); ?>" class="plugin-to-install">
				<label for="plugin-import[<?php echo esc_attr( $slug ); ?>]">
					...
					$plugin_details = ( ( isset( $plugin['details_url'] ) && '' != $plugin['details_url'] ) ? sprintf( ' (<a href="%s" target="_blank" rel="noopener">%s</a>)', esc_url( $plugin['details_url'] ), esc_html__( 'View details', 'ciyashop' ) ) : '' );
					echo sprintf(
		...
		public function envato_setup_customize() {
			?>
			<h1><?php printf( esc_html__( 'Setup %s Child Theme (Optional)', 'ciyashop' ), $this->theme_data->get( 'Name' ) ); ?></h1>
		...
		private function _make_child_theme( $child_theme_title ) {
			...
			if ( ! file_exists( $child_theme_path ) || ( file_exists( $child_theme_path ) && $force_child_creation ) ) {
				...
				if ( file_exists( $child_theme_path ) && $force_child_creation ) {
					$child_theme_bak_path = $child_theme_path . '_' . date( 'Y-m-d_H-i-s' );
			...
			if ( $parent_theme_template !== $child_theme_slug ) {
				...
				<?php printf( esc_html__( 'Child Theme %1$s created and activated! Folder is located in wp-content/themes/%2$s', 'ciyashop' ), '<strong>' . $child_theme_title . '</strong>', '<strong>' . $child_theme_slug . '</strong>' ); ?></h1>
				</p>
				<?php
				...
				<div class="envato-setup-final-content envato-setup-final-content-last">
					<div class="envato-setup-final-content-inner">
						...
						<div class="more-resources">
							<div class="more-resource documentation"><a href="http://docs.potenzaglobalsolutions.com/docs/ciyashop-wp/" target="_blank" rel="noopener"><?php esc_html_e( 'Read the Theme Documentation', 'ciyashop' ); ?></a></div>
							<div class="more-resource howto"><a href="https://wordpress.org/support/" target="_blank" rel="noopener"><?php esc_html_e( 'Learn how to use WordPress', 'ciyashop' ); ?></a></div>
							<div class="more-resource rating"><a href="https://themeforest.net/downloads" target="_blank" rel="noopener"><?php esc_html_e( 'Leave an Item Rating', 'ciyashop' ); ?></a></div>
							<div class="more-resource support"><a href="https://potezasupport.ticksy.com/" target="_blank" rel="noopener"><?php esc_html_e( 'Get Help and Support', 'ciyashop' ); ?></a></div>
						</div>
					</div>
				</div>
			</div>
...
function ciyashop_hide_setup_wizard_notice() {
	...
	if ( isset( $_GET['ciyashop-setup-hide-notice'] ) && isset( $_GET['_cssetup_notice_nonce'] ) ) { // WPCS: input var ok, CSRF ok.
		if ( ! wp_verify_nonce( sanitize_key( $_GET['_cssetup_notice_nonce'] ), 'ciyashop_setup_hide_notice_nonce' ) ) { // WPCS: input var ok, CSRF ok.
		...
		$hide_notice = sanitize_text_field( $_GET['ciyashop-setup-hide-notice'] );
