...
function ciyashop_product_actions_add_variation_product_view() {
	...
	if ( empty( $id ) || ! $product->is_type( 'variable' ) || ! isset( $ciyashop_options['cs_display_variation_on_list'] ) || false == $ciyashop_options['cs_display_variation_on_list'] ) {
		return;
	}
	?>
...
if ( ! function_exists( 'ciyashop_load_variations' ) ) {
	...
	function ciyashop_load_variations( $id = false ) {
		...
		foreach ( $variable_posts as $post ) :
			setup_postdata( $post );
			if ( ! is_product() ) {
...
if ( ! function_exists( 'ciyashop_attr_variation_list' ) ) {
	function ciyashop_attr_variation_list( $attribute_name = false ) {
		...
		if ( empty( $attribute_name ) || null == $attribute_name ) {
			return false;
		}
		...
		foreach ( $attributes_to_show as $key => $swatch ) {
			...
			?>
				...
				<a class="<?php echo esc_attr( $class ); ?>" style="<?php echo esc_attr( $style ); ?>" title="<?php echo esc_attr( ucwords( $term->name ) ); ?>" <?php echo $data; ?>>
