...
if ( ! function_exists( 'ciyashop_product_sale_countdown' ) ) {
	function ciyashop_product_sale_countdown() {
		...
		if ( is_single() && get_post_type() == 'product' && 0 == $ciyashop_options['product_countdown'] ) {
			return;
		} elseif ( ! is_single() && 0 == $ciyashop_options['shop_countdown'] ) {
			return;
		}
...
function ciyashop_get_swap_image( $size = 'woocommerce_thumbnail' ) {
	...
	if ( isset( $ciyashop_options['product_image_swap'] ) && 1 == $ciyashop_options['product_image_swap'] ) {
		$attachment_ids = $product->get_gallery_image_ids();
