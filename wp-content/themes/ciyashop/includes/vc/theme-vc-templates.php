...
function ciyashop_get_vc_templates() {
	...
	if ( file_exists( $templates_list_path ) ) {
		$templates_list_raw = include( $templates_list_path );
	...
	foreach ( $templates_list as $template_name ) {
		...
		if ( file_exists( $template_file ) ) {
			...
			if ( $template_data && is_array( $template_data ) && ( isset( $template_data['name'] ) && '' != $template_data['name'] ) && ( isset( $template_data['content'] ) && '' != $template_data['content'] ) ) {
				...
				$template_data['template_category_slug'] = $template_category_slug = sanitize_title( $template_data['template_category'] );
