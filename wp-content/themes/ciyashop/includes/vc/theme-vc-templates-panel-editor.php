...
class CiyaShop_Vc_Templates_Panel_Editor {
	 ...
	 * @return string
	 */
	public function renderTemplateWindowCsVcTemplates( $template_name, $template_data ) {
		ob_start();
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: CiyaShop_Vc_Templates_Panel_Editor::renderBackendTemplate:1' );
		}
	...
	public function getBackendDefaultTemplate( $return = false ) {
		...
		if ( ! $data ) {
			die( 'Error: CiyaShop_Vc_Templates_Panel_Editor::getBackendDefaultTemplate:1' );
		}
		if ( $return ) {
