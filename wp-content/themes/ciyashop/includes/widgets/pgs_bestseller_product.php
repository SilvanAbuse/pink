...
if ( ! class_exists( 'PGS_Bestseller_Widget' ) ) {
	class PGS_Bestseller_Widget extends WC_Widget {
		...
		function __construct() {
			...
			$this->widget_name        = esc_html__( 'PGS Bestseller Products', 'ciyashop' );
			$this->widget_description = esc_html__( 'Display bestseller products in sidebar.', 'ciyashop' );
			$this->widget_cssclass    = 'widget_pgs_bestseller_widget';

			$this->settings = array(
				'title'       => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Bestseller Products', 'ciyashop' ),
				...
				'num_of_item' => array(
					...
					'max'   => '',
					'std'   => 8,
					'label' => esc_html__( 'Number of item', 'ciyashop' ),
				),
			);
		...
		function get_bestseller_products( $args, $instance ) {
		...
		public function widget( $args, $instance ) {
			...
			if ( $bestseller_products && count( $bestseller_products ) > 0 ) {
				...
				// $products = apply_filters( 'ciyashop_bestseller_product_widget_bestseller_products', $products );
											...
											<div class="item-img">
												<a href="<?php the_permalink( $product_id ); ?>" title="<?php the_title_attribute( array( 'post' => $product_id ) ); ?>">
													...
													if ( has_post_thumbnail( $product_id ) ) {
														...
														if ( isset( $ciyashop_options['enable_lazyload'] ) && $ciyashop_options['enable_lazyload'] ) {
															echo '<img class="img-fluid owl-lazy" src="' . esc_url( LOADER_IMAGE ) . '" data-src="' . esc_url( get_parent_theme_file_uri( '/assets/img/placeholder/shop_thumbnail.png' ) ) . '" alt="' . esc_attr__( 'No thumb', 'ciyashop' ) . '"/>';
														} else {
															echo '<img class="img-fluid" src="' . esc_url( get_parent_theme_file_uri( '/assets/img/placeholder/shop_thumbnail.png' ) ) . '" alt="' . esc_attr__( 'No thumb', 'ciyashop' ) . '"/>';
														}
													}
													?>
												</a>
