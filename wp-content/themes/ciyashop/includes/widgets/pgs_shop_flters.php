...
class PGS_Shop_Filters_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops  = array(
			...
			'description'                 => esc_html__( 'Display product filters in your store (for horizontal use only and will be available for shop/product listing slidebar only).', 'ciyashop' ),
	...
	public function widget( $args, $instance ) {
		...
		*/
		...
		} elseif ( ! isset( $instance['show_in'] ) || 'shop_pg_content' != $instance['show_in'] ) { // called from widget
			$title = esc_attr__( 'Shop Filter', 'ciyashop' );
		}
		...
		<div class="pgs-shop-filters-wrapper">
			<div class="row no-gutters">
				<?php
				if ( in_array( 'search-box', $filters_to_show ) ) {
					?>
				...
				if ( in_array( 'ratings', $filters_to_show ) ) {
				...
				if ( in_array( 'price-slider', $filters_to_show ) ) {
					?>
				<div class="col-md-4 col-sm-6">
					<div class="shop-filter shop-filter-product-price widget_price_filter">
						<?php
						if ( ! isset( $instance['show_in'] ) || 'shop_pg_content' != $instance['show_in'] ) {
							?>
						<form>
							<?php
						}
								...
								<div class="price_slider_wrapper">
									<div class="price_slider_wrapper-inner">
										...
										<div class="price_slider_amount">
											<input type="text" id="min_price" name="min_price" value="<?php echo esc_attr( $min_price ); ?>" data-min="<?php
																													/**
																													 * Filters shop price-filter minimum amount.
																													 *
																													 * @visible false
																													 * @ignore
																													 */
																													echo esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) );
																													?>" placeholder="<?php echo esc_attr__( 'Min price', 'ciyashop' ); ?>" />
											<input type="text" id="max_price" name="max_price" value="<?php echo esc_attr( $max_price ); ?>" data-max="<?php
																													 ...
																													 * @ignore
																													 */
																													echo esc_attr( apply_filters( 'woocommerce_price_filter_widget_max_amount', $max ) );
																													?>" placeholder="<?php echo esc_attr__( 'Max price', 'ciyashop' ); ?>" />
									...
									<button type="submit" class="button"><?php esc_html_e( 'Filter', 'ciyashop' ); ?></button>
								</div>
							</div>
						<?php
						if ( ! isset( $instance['show_in'] ) || 'shop_pg_content' != $instance['show_in'] ) {
					...
					foreach ( $attribute_taxonomies as $tax ) {
						...
						if ( taxonomy_exists( wc_attribute_taxonomy_name( $tax->attribute_name ) ) ) {
							...
							if ( 0 !== sizeof( $terms ) ) {
								...
								$row_break = 6;
								if ( 0 != $attribute_sr && 0 == $attribute_sr % $row_break ) {
									?>
									</div>
									<div class="row no-gutters">
									<?php
								...
								?>
								<div class="col-sm">
									<div class="shop-filter shop-filter-product-<?php echo esc_attr( $tax->attribute_name ); ?>" data-placeholder="<?php echo sprintf( esc_attr__( 'Any %s', 'ciyashop' ), esc_html( $taxonomy_label ) ); ?>">
										<div class="shop-filter-wrapper">
	...
	public function form( $instance ) {
		...
		$filter_btn_title = ! empty( $instance['filter-btn-title'] ) ? $instance['filter-btn-title'] : esc_html__( 'Filter', 'ciyashop' );
		?>
		<div class="ciyashop-filter-widget">
			...
			<div class="form-group">
				<label for="<?php echo esc_attr( $this->get_field_id( 'filter-attributes' ) ); ?>">
					<?php esc_html_e( 'Select Filter fields:', 'ciyashop' ); ?>
				</label>
				<p>
				<?php
				...
				if ( ! empty( $attributes ) ) {
					foreach ( $attributes as $key => $attr ) {
						$checked = ( ! empty( $selected_attrs ) && in_array( $key, $selected_attrs ) ) ? 'checked="checked"' : '';
						?>
						<span>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'filter-attributes' ) ); ?>">
					<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=product&page=product_attributes' ) ); ?>" target="_blank">
						<?php esc_html_e( 'Select Filter Attributes:', 'ciyashop' ); ?>
					</a>
				</label>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['filter-attributes'] = ( ! empty( $new_instance['filter-attributes'] ) ) ? strip_tags( json_encode( $new_instance['filter-attributes'] ) ) : '';
