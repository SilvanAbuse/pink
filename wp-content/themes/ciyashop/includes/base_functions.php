						...
						if ( ! $l['active'] ) {
							...
							<li>
								<a href="<?php echo esc_url( $l['url'] ); ?>">
									...
									if ( isset( $ls_settings['icl_lso_flags'] ) && 1 == $ls_settings['icl_lso_flags'] ) {
...
function ciyashop_header_type() {
	...
	if ( isset( $ciyashop_options['header_type'] ) && 'header_builder' == $ciyashop_options['header_type_select'] ) {
...
function ciyashop_get_file_list( $extensions = '', $path = '' ) {
	...
	if ( defined( 'GLOB_BRACE' ) ) {
		...
		if ( 1 == count( $extensions ) && '*' == $extensions[0] ) {
...
function ciyashop_pgscore_update_notice() {
	...
	if (
		( ! defined( 'PGSCORE_VERSION' ) || ( defined( 'PGSCORE_VERSION' ) && version_compare( PGSCORE_VERSION, '4.3.0', '<' ) ) )
	) {
		$ciyashop_token = ciyashop_is_activated();
		if ( $ciyashop_token ) {
			$title     = esc_html__( 'Update PGS Core', 'ciyashop' );
			$msg       = '<p>' . __( 'Please ensure to update PGS Core plugin to latest version to enable features.', 'ciyashop' );
		...
		<div id="setting-error-pgscore_update" class="notice notice-error">
			<h2 class="setting-error-pgscore_update-header"><i class="dashicons dashicons-warning"></i> <?php echo esc_html( $title ); ?></h2>
...
function ciyashop_main_content_container_classes( $classes ) {
	...
	if ( is_post_type_archive( 'portfolio' ) && 'full_width' == $ciyashop_options['portfolio_sidebar'] && 1 == $ciyashop_options['portfolio_fullscreen'] ) {
...
function ciyashop_related_portfolio() {
	...
	if ( isset( $ciyashop_options['display_related_portfolio'] ) && 1 == $ciyashop_options['display_related_portfolio'] ) {
...
if ( ! function_exists( 'ciyashop_save_custome_sidebar_metabox' ) ) {
	function ciyashop_save_custome_sidebar_metabox( $post_id ) {
		if ( ! isset( $_POST['pgs_custom_sidebar_nonce'] ) || ! wp_verify_nonce( $_POST['pgs_custom_sidebar_nonce'], 'pgs_custom_sidebar_metabox_nonce' ) ) {
			return;
		}
		...
		if ( isset( $_POST['pgs_custom_sidebar'] ) ) {
			update_post_meta( $post_id, 'pgs_custom_sidebar', sanitize_text_field( $_POST['pgs_custom_sidebar'] ) );
		}
	}
}
				...
				foreach ( $ciyashop_redux_sections as $section ) {
					...
					if ( isset( $section['title'] ) ) {
						...
						if ( isset( $section['icon'] ) ) {
							$current_icon = $option_fields[ $index ]['icon'] = $section['icon'];
						} else {
	...
	function ciyashop_breadcrumbs( $class = '' ) {
		...
		if ( is_archive() && ! is_tax() && ! is_category() && ! is_tag() && ! is_author() ) {
			...
			elseif ( is_year() ) :
				...
				$output .= post_type_archive_title( $prefix = '', false );
			endif;
			$output .= '</li>';
