	...
	class CiyaShop_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( 0 == $depth ) {
				...
				if ( 'mega-menu' == $menu_design ) {

					if ( '' != $menu_columns && '' == $html_block_id ) {
					...
					if ( 'custom-size' == $mega_menu_design ) {
					...
					if ( '' != $html_block_id && function_exists( 'pgs_get_html_block' ) ) {
				...
				if ( '' != $menu_widget_area && 1 == $depth ) {
					...
					if ( 'mega-menu' == $parent_mega_menu_design ) {
			...
			if ( '' != $menu_icon && 'fip-icon-block' != $menu_icon ) {
			...
			if ( '' != $styles && 'site-mobile-menu' != $args->menu_class ) {
