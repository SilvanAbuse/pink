...
function ciyashop_activate_theme() {
	...
	if ( isset( $_POST['ciyashop_purchase_code'] ) && isset( $_POST['purchase_code_nonce'] ) ) {
		...
		if ( wp_verify_nonce( $_POST['purchase_code_nonce'], 'purchase_code_activation' ) ) {
			$ciyashop_purchase_code = sanitize_text_field( $_POST['ciyashop_purchase_code'] ); // Sanitize purchase code.
			...
			if ( empty( $ciyashop_purchase_code ) ) {
				...
				if ( true !== ciyashop_prevalidate_purchase_code_JYqed( $ciyashop_purchase_code ) ) {
					delete_option( 'ciyashop_theme_purchase_key' );
					delete_option( 'ciyashop_pgs_token' );
					$notices = array(
						'notice_type' => 'warning',
					...
					if ( ( $ciyashop_purchase_code_old == $ciyashop_purchase_code ) // check old code and new code are same.
						&& ( $ciyashop_site_url_key_old == $ciyashop_site_url_key ) // check if old url is same as new one.
							...
							if ( 200 == $response_code ) {
								...
								$notices = array(
									...
									'notice'      => sprintf(
										...
										esc_html__( 'There was an error processing your request with http status code %d; please try again later.' ),
										$response_code
									),
								);
							}
 ...
 */
function ciyashop_prevalidate_purchase_code_JYqed( $purchase_code ) {
