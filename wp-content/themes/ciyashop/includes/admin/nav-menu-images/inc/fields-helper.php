...
function menu_render_fields( $item_id, $menu_depth = '', $parent_menu_id = '' ) {
	...
	foreach ( $field_args as $args ) {
		if ( 1 == $menu_depth && ! empty( $parent_menu_id ) && 'menu_widget_area' == $args['id'] ) {
			$parent_saved = get_post_meta( $parent_menu_id, 'pgs_menu-item-menu_design', true );

			if ( 'mega-menu' == $parent_saved ) {
...
function get_menu_fields_html( $item_id, $args, $saved ) {
	...
	$pgs_menu_depth = $args['depth'];

	ob_start();

	?>
	<div class="<?php echo esc_attr( $field_wrapper_class ); ?>" data-menu-depth="<?php echo esc_attr( json_encode( $pgs_menu_depth ) ); ?>">
	<?php
	...
	switch ( $field_type ) {
		...
		case 'textfield':
			...
			<div class="pgs_menu_input-field">
				...
				if ( isset( $args['desc'] ) && ! empty( $args['desc'] ) ) {
					?>
					<span class="description"><?php echo $args['desc']; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotE ?></span>
					<?php
				}
				?>
			</div>
		...
		<div class="pgs_menu_input-field pgs_menu_input-field-image">
			...
			echo get_menu_field_settings( $item_id );
